...
JB.setgc = function() {  
	...
	JB.gc.hglattlaen = (typeof(Hglattlaen)!="undefined") ? Hglattlaen : 10; // in Meter
	JB.gc.vglattlaen = (typeof(Vglattlaen)!="undefined") ? Vglattlaen : 10; // in Meter
	...
	JB.gc.profilfillopac = (typeof(Profilfillopac)!="undefined") ? Profilfillopac : 0.35; //   0 ... 1, 0:aus
...
JB.GPX2GM.start = function() {
	...
	JB.LoadScript(JB.GPX2GM.Path+"GPX2GM_Defs.js", function() { 
		...
		JB.addMap = function(id){
			var length = maps.length;

			maps["Karte_"+length] = div.makeMap = new JB.makeMap(Id);

			return length;

		}

		JB.showMap = function(filename,typ){
			var filenames = [filename];
			maps["Karte_"+Id + '0'].ShowGPX(filenames,typ);
		};

		JB.showMap = function(trackArray,type){
			var filenames = [];
			angular.forEach(trackArray, function(value, key) {
				filenames.push(value.filename);
			});

			maps["Karte_"+Id].ShowGPX(filenames,type);
		};
